...
function TaskImage({ imageUrl }: { imageUrl?: string | null }) {
  if (imageUrl) {
    return (
      <img src={imageUrl} alt="" className="task-card-image" loading="lazy" />
    );
  }
...
export function HomePage() {
  ...
  const handleSubmitTask = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        const uploaded = await uploadPublicImage(imageFile, "tasks");
                ...
                <label className="task-form__field task-form__field--full">
                  ...
                  {imagePreviewUrl ? (
                    <img
                      src={imagePreviewUrl}
                      alt=""
                      className="task-form__image-preview"
                    />
                  ) : null}
                </label>
                <label className="task-form__field">
